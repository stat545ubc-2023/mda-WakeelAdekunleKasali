...
I chose the four datasets because their descriptions attract my interest. 
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
I chose the "cancer_sample" dataset because all the variables have values that
are numerical, and so it would be easy to create more variables from the ones
in the datasets and perhaps carry out correlation analysis.
<!----------------------------------------------------------------------------->
...
irregularity in the shape of tumor cell ) and compactness_mean (representing 
closeness of packed cells) in the cancer_sample dataset?
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
1. I have chosen plotting the distribution of the "flow" variable in the 
"flow_sample" data, because that enables me to observe behaviour of flow
across years. dyplr functions like filter(), group_by(), pipe function and
ggplot make this possible. This helps in detecting some measures like mean and 
variance(spread) of the "flow" which is very crucial in understanding the 
feautures of the dataset.

2. I have chosen creating a new variable "year_difference" in "apt_building" 
dataset because the difference between the year a building was registered and 
when it was built will help in understanding how long it takes a building to 
get a legal warrant. I used a mutate(), select() and pipe functions in getting 
it done. It might make government get an informed decision.
...
(closeness of the tumor cells) in cancer_sample because it might enhance 
further understanding about the cancer. I used ggplot.

4. I have chosen to narrow my search to the distribution of trees in 
MARPOLE  neighbourhood with street_side_names that are ODD in "vancouver_trees" 
dataset because the city officials might seem interested in the part of the 
data specific to that. I combined the filter(), select() and pipe functions.
...
<!--- *****START HERE***** --->
1. How does the distribution of the "flow" variable change in every 10 years 
in the "flow_sample" dataset? This will break the year date to every decade(ten
 years).This might involve using group_by(), select, count(),summary() and pipe
 functions, including ggplot()
...
3. What is the form of relationship - positive or negative - between the 
fractal_dimension_mean and compactness_mean using "cancer_sample"? This might 
attract coding for fitting regression line.

4. For the purpose of urban planning and using vancouver_tree dataset, what is 
the distribution pattern of trees in KENSINGTON-CEDAR COTTAGE neighbourhood 
having a street_side_name that are even. This would take filter(), select(), 
and some other dyplyr functions including ggplot.
<!----------------------------->
